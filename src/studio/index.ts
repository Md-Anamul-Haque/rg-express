...
export async function studio(_app: typeof expressApp, startDirName: string, lang: 'ts' | 'js') {
    ...
    if (lang == 'ts') {
        ...
        codes.push("router.get('/_rg/studio/apis', (_req: Request, res: Response) => {")
    } else {
        codes.push(`let apiUrls = ["${apiUrls.join('","')}"]`)
        codes.push("router.get('/_rg/studio/apis', (_req, res) => {")
    }
    codes.push(`
    const listItems = apiUrls.map(item => \`<li> <a href="\${item.split(':').slice(1).join(':')}">\${item}</a></li>\`).join('');
    `)
    codes.push(`
    ...
    if (lang == 'ts') {
        codes.push(`\nrouter.get('/_rg/apis/:method', (req: Request, res: Response) => {`);
    } else {
        codes.push(`\nrouter.get('/_rg/apis/:method', (req, res) => {`);
